...
# from models.STDAN.sttrans import STTrans2 as STVSR      ## STDAN
...
parser.add_argument('--data_dir', type=str, default='/home/chl/PycharmProjects/dataset/189_vime7/train/GT')
parser.add_argument('--file_list', type=str, default='189_vime7_train.txt')
parser.add_argument('--patch_size', type=int, default=64, help='0 to use original frame size')

# -- Test while Training ———
parser.add_argument('--test_dir', type=str, default='./test_example_FISR/*')

parser.add_argument('--pretrained', type=bool, default=False)
parser.add_argument('--save_folder', default='weights/', help='Location to save checkpoint models')
parser.add_argument('--pretrained_sr', default='4x_cc-system-product-name_epoch_71.pth',
                    help='sr pretrained base model default=4x_DESKTOP-0NFK80A_epoch_2.pth')
...
def save_best_model(bestepoch):
    model_out_path = args.save_folder + 'best_' + str('4x_' + hostname + "_epoch_{}.pth".format(bestepoch))
    torch.save(model.state_dict(), model_out_path)
...
# model = STVSR(n_inputs=7, nf=64, embed_dim=72)
print_network(model)
...
if args.pretrained:
    ...
    if os.path.exists(model_name):
        ...
        print("pretrained_sr is non-existent")
        exit()
...
training_data_loader = DataLoader(dataset=train_set, num_workers=12, batch_size=args.batchSize, shuffle=True, pin_memory=True)

# -- begin training --
best_epoch = 0
best_test_psnr = 0.0

fw = open('runs/MSTT_STVSR/train.txt', 'a')
...
for epoch in range(args.start_epoch, args.nEpochs + 1):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        print("===> Epoch[{}]({}/{}): Loss: {:.4f} || Timer: {:.4f} sec.".format(epoch, iteration,
                                                                                 ...
                                                                                 loss.item(),
                                                                                 (t1 - t0)))
        # fw.write('===> Epoch[{}]({}/{}): Loss: {:.4f} || Timer: {:.4f} sec.'.format(epoch, iteration, len(training_data_loader), loss.item(), (t1 - t0)))
        # fw.write("\n")
    print("===> Epoch {} Complete: Avg. Loss: {:.4f}".format(epoch, epoch_loss / len(training_data_loader)))
    # fw.write("\n")
    fw.write("===> Epoch {} Complete: Avg. Loss: {:.4f}".format(epoch, epoch_loss / len(training_data_loader)))
    fw.write("\n")
    ...
    fw.write('===> Epoch {} Complete: Test avg.PSNR: {:.4f} dB'.format(epoch, avg_test_psnr))
    fw.write("\n\n")
    ...
    if epoch == args.nEpochs:
        ...
        fw.write('Best_epoch:{:.4f},Best_psnr={:.6f}'.format(best_epoch, best_test_psnr))
        fw.write("\n")
